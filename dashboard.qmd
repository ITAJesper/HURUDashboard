...
def plot():
    ...
    logger.info(f"✅ [PLOT] Valgt aktivitet: {selected_event}")
    logger.info(f"✅ [PLOT] Dato-interval: {start_date} til {end_date}")
    ...
    logger.info(f"✅ [PLOT] Rækker i df_filtered: {len(df_filtered)}")
    logger.info(df_filtered[["Event_Name", "Event_Date"]].head().to_string())
    ...
    logger.info(f"✅ [PLOT] Rækker i df_long: {len(df_long)}")
    logger.info(df_long.head().to_string())
